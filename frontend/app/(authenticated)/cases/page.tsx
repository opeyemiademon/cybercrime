        ...
        {!isLoading && !error && (
          <>
                ...
                >
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center space-x-2 ml-4">
                      <button
                        onClick={() => setUploadingEvidence(caseItem)}
            ...
            return (
              <div 
                key={caseItem.id}
                className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-5 hover:shadow-lg transition-all duration-300 cursor-pointer animate-fadeIn"
                style={{ animationDelay: `${index * 50}ms` }}
              >
...
function UploadEvidenceModal({ caseData, onClose }: { caseData: CaseData; onClose: () => void }) {
  const queryClient = useQueryClient();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.hash) {
      setError('Please provide the file hash for integrity verification');
      return;
    }
    ...
    try {
      const evidenceData: CreateEvidenceData = {
        ...
        location: formData.location,
        notes: formData.notes,
        filePath: `/evidence/${caseData.id}/${formData.filename}`,
      };

      await createEvidenceMutation.mutateAsync(evidenceData);
      onClose();
    } catch (err: any) {
  ...
  const isLoading = createEvidenceMutation.isPending;
        ...
        <div className="p-6 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400">Case: {caseData.caseId}</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-primary-50 file:text-primary-700 hover:file:bg-primary-100 dark:file:bg-primary-900 dark:file:text-primary-300"
              required
            />
            {file && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              ...
              <Select
                value={{ value: formData.evidenceType, label: formData.evidenceType }}
                onChange={(option) => setFormData({ ...formData, evidenceType: option?.value })}
                options={[
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Hash Algorithm *</label>
              <Select
                value={{ value: formData.hashAlgorithm, label: formData.hashAlgorithm }}
                onChange={(option) => setFormData({ ...formData, hashAlgorithm: option?.value })}
                options={[
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">File Hash *</label>
            <input
              type="text"
              value={formData.hash}
              onChange={(e) => setFormData({ ...formData, hash: e.target.value })}
              className="w-full px-4 py-3 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none transition-all font-mono text-sm"
              placeholder="Enter file hash for integrity verification"
              required
            />
            <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
              Generate hash using: <code className="bg-gray-100 dark:bg-gray-700 px-1 py-0.5 rounded">shasum -a 256 filename</code>
            </p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Source Device</label>
              <input
                type="text"
                value={formData.sourceDevice}
                onChange={(e) => setFormData({ ...formData, sourceDevice: e.target.value })}
                className="w-full px-4 py-3 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none transition-all"
                placeholder="e.g., Laptop-001, Phone-XYZ"
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Collection Location</label>
              <input
                type="text"
                value={formData.location}
                onChange={(e) => setFormData({ ...formData, location: e.target.value })}
          ...
          <div>
            ...
            <textarea
              value={formData.notes}
              onChange={(e) => setFormData({ ...formData, notes: e.target.value })}
              className="w-full px-4 py-3 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none transition-all resize-none"
              rows={3}
          ...
          <div className="flex justify-end space-x-3 pt-4">
            ...
            <button 
              type="submit" 
              disabled={isLoading}
              className="px-6 py-3 bg-green-600 dark:bg-green-500 text-white rounded-lg hover:bg-green-700 dark:hover:bg-green-600 transition-all font-medium shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2"
            >
              {isLoading ? (
                <>
