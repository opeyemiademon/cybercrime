  ...
  const initializeMedia = useCallback(
    async (mode: 'user' | 'environment' = facingMode) => {
      ...
      try {
        ...
        console.log('Media stream obtained. Tracks:', mediaStream.getTracks().map(t => ({
          kind: t.kind,
          label: t.label,
          enabled: t.enabled,
          muted: t.muted,
    ...
    try {
      ...
      mediaRecorder.onstop = () => {
        const blob = new Blob(chunksRef.current, { 
          type: mimeType || (captureType === 'video' ? 'video/webm' : 'audio/webm')
        });
              ...
              <div className='relative aspect-[4/3] w-full'>
                {captureType === 'audio' ? (
                  <div className='absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-blue-900 to-purple-900 text-white'>
                    ...
                    {recordedAudio && (
                      <audio src={recordedAudio} controls controlsList="nodownload" className='mt-4 w-full max-w-md' />
                    )}
                  </div>
                ) : (
                  <>
