...
export const verifyJWTToken = (token: string): JWTPayload => {
  try {
    const decoded = jwt.verify(token, include.TOKEN_SECRET) as JWTPayload;
    return decoded;
  } catch (error) {
...
export const getUserFromToken = async (payload: JWTPayload): Promise<AuthenticatedUser | null> => {
  try {
...
export const requireAuth = (context: AuthContext): AuthenticatedUser => {
