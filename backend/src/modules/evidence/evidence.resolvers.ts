...
const generateEvidenceId = async (): Promise<string> => {
...
const evidenceResolvers = {
  Query: {
    getEvidence: async (_: any, { id }: { id: string }, context: any) => {
      requireAuth(context);
      try {
    ...
    getAllEvidence: async (_: any, { caseId, status, search }: any, context: any) => {
      requireAuth(context);
      try {
    ...
    getEvidenceByCase: async (_: any, { caseId }: { caseId: string }, context: any) => {
      requireAuth(context);
      try {
  ...
  Mutation: {
    createEvidence: async (_: any, { input }: any, context: any) => {
      ...
      try {
        ...
        if (input.fileData) {
          try {
            uploadedFile = await processEvidenceUpload(
              input.fileData,
              input.caseId,
              input.filename,
              evidenceId
    ...
    updateEvidence: async (_: any, { id, input }: any, context: any) => {
      requireAuth(context);
      try {
        const evidence = await Evidence.findById(id);
        if (!evidence || evidence.isDeleted) {
    ...
    verifyEvidence: async (_: any, { input }: any, context: any) => {
      const user = requireAuth(context);
      try {
